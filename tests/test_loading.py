...
from aioquerysaur.loader import QueryLoader
...
def test_load_dir(mocker, sql_dir):
    mock_load_from_tree = mocker.patch('aioquerysaur.queries.QueriesContainer.load_from_tree')
    load_from_file(sql_dir, 'psycopg2')
    mock_load_from_tree.assert_called_once()


def test_load_file(mocker, sql_file):
    mock_load_from_list = mocker.patch('aioquerysaur.queries.QueriesContainer.load_from_list')
